...
import datetime
import sqlite3
from flask import Blueprint, g, render_template, redirect, request, url_for, jsonify

# Blueprint 생성
costco_bp = Blueprint('costco', __name__, url_prefix='/costco')
...
@costco_bp.teardown_app_request
def 연결_닫기(예외):
    디비 = getattr(g, '_database', None)
    if 디비 is not None:
        디비.close()
...
@costco_bp.route('/')
def 루트():
    return redirect('/costco/purchase')

# 날짜/매장 선택 페이지
@costco_bp.route('/purchase/select', methods=['GET'])
def 날짜_매장_선택():
    디비 = 데이터베이스_가져오기()
...
def 품목_상세_수정(date, store_code, item_id):
    ...
    if request.method == 'POST':
        if item_id != 0 and request.form.get('delete') == '1':
            ...
            디비.commit()
            return redirect(url_for('costco.구매일자별_매장별_구매목록', date=date, store_code=store_code))
